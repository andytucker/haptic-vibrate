...
- Prefer the existing release builder (`composer run build-release`, which runs `scripts/build-release.php`) so the archive is created from `dist/package/haptic-vibrate` with portable paths.
- Before considering release packaging complete, verify the ZIP opens successfully and contains the plugin under a top-level `haptic-vibrate/` directory with readable files.
